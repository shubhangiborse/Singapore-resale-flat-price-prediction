...
st.markdown("<h1 style='text-align: center; color:orange;'>Singapore Flat Resale Price Predicton</h1> By Shubhangi Borse",unsafe_allow_html=True)
...
with tab2:
    ...
    if option == 'Processed Data':
        st.header("Data after Processed_final")
...
with tab3:
    ...
    if st.button('Predict and Conclusion'):
        ...
        st.success(f"Resale Price: :orange[S$] :orange[{predicted_price}]")  

        st.markdown('### :violet[Some Insights...]')
        st.write('* Resale price of the flats are majorly depends on the number of:orange[storeys in the flat, Flat type, Floor area].')
        ...
        st.write('* :orange[Population growth, changes in demographics], and:orange[urban development plans] can influence the demand for housing in certain areas.')
